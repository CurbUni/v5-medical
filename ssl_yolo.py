...
class BYOL(nn.Module):
    def __init__(self, backbone):
        ...
        self.prediction_head = BYOLProjectionHead(512, 1024, 512)

        # self.projection_head = BYOLProjectionHead(1024, 2048, 1024)
    ...
    def forward(self, x):
                                                   # x: 32, 3, 640, 640 
        y = self.backbone(x).flatten(start_dim=1)  # output y: 16,1024   # 16,256 
        z = self.projection_head(y)  # 16,1024  # 16,256  
        p = self.prediction_head(z)  # 16,1024  # 16,256 
        return p
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default='./models/segment/yolov5l-seg-own.yaml', help='model.yaml')  # ./models/segment/yolov5l-seg.yaml  # ./models/yolov5s-final.yaml
    ...
    # dataset = LightlyDataset("./datasets/ChrisDataSet/images/train2017")
    dataset = LightlyDataset("./datasets/coco_OrCaScoreDataSet_1/images/train")
    ...
    for epoch in range(100):
        ...
        if best_avg_loss > avg_loss:
            torch.save(model.backbone.state_dict(), "new_best_yolopBackbone.pth")
