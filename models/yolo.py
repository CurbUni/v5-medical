...
Usage:
    ...
    python models/yolo.py --cfg yolov5l-seg.yaml
"""
import sys
...
from models.exp_module import BFF, FAM_IFM, InjectionMultiSum_Auto_pool, QueenFusion
try:
...
class YoloBackbone(BaseModel):
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        x = self._forward_once(x, profile, visualize)  # x: 32, 256, 40 ,40
        x = self.avgpool(x)   # x: 32, 256, 1 ,1
        # x = torch.flatten(x, 1)
        # x = self.fc(x)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, BFF, DRConv2d}:
            c1, c2 = ch[f], args[0]
            ...
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x}:
                ...
                n = 1
        elif m is QueenFusion:
            args = [make_divisible(x * gw, 8) for x in args]
            c2 = args[1]
        ...
        elif m in {repvit_m1, repvit_m2, repvit_m3}:
            m = m(*args)
            c2 = m.channel
        elif m in {FAM_IFM, InjectionMultiSum_Auto_pool}:
            c2 = 192
        else:
            c2 = ch[f]
        ...
        m_.i, m_.f, m_.type, m_.np = i + 4 if is_backbone else i, f, t, np
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        save.extend(x % (i + 4 if is_backbone else i)
                    for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist
        layers.append(m_)
        if i == 0:
            ch = []
        if isinstance(c2, list):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='segment/yolov5l-seg-own.yaml', help='model.yaml')
