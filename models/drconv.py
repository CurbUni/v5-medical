...
class DRConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, region_num=8, **kwargs):
        ...
        # self.resize = F.interpolate(tensor, size=target_size, mode='bilinear', align_corners=False)
    ...
    def forward(self, input):
        ...
        target_size = (input.size(2), input.size(3))
        output = F.interpolate(output, target_size, mode='bilinear', align_corners=False)
        return output
...
if __name__ == '__main__':
    ...
    conv.train()
    input = torch.ones(B, in_channels, size, size).cuda() # 16, 256, 89, 89
    output = conv(input)
